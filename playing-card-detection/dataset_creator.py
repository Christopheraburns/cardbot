...
cardW *= mm2pix
cardH *= mm2pix
cornerXmin=int(cornerXmin*mm2pix)
cornerXmax=int(cornerXmax*mm2pix)
cornerYmin=int(cornerYmin*mm2pix)
cornerYmax=int(cornerYmax*mm2pix)



def display_img(img,polygons=[],channels="bgr",size=8):
    """
...
if os.path.isfile("data/backgrounds.pck"):
    print("Background file already created")
else:
    dtd_dir = "dtd/images/"
    bg_images = []
    ...
    print("# of images loaded: {}".format(len(bg_images)))
    print("Saved in: {}".format(backgrounds_pck_fn))
...
class Backgrounds():
    def __init__(self, backgrounds_pck_fn=backgrounds_pck_fn):
        ...
        print("# of images loaded: {}".format(self._nb_images))
...
#print("loading test background image...")
...
def varianceOfLaplacian(img):
    ...
    print("Lablacian: {}".format(val))
    return val

def extract_card(img, output_fn=None, min_focus=87, debug=False):
    imgwarp=None
...
def extract_cards_from_pics(image_fn=None, output_dir=None, keep_ration=5, min_focus=97, debug=False):
    cardcount = 0
    for f in glob("data\Cards\*.jpg"):
        split = f.split("\\")
        fullname = split[len(split) - 1]
        i = fullname.find('.')
        filename = fullname[:i]
        img = cv2.imread(f)
        valid, card = extract_card(img)
        newname = "data\\" + filename + ".png"
        if valid:
            cv2.imwrite(newname, card)
        else:
            print("{} did not process".format(f))
        cardcount += 1
        print("processed {} cards".format(cardcount))
...
#extract_cards_from_pics()

imgs_dir="data"

if not os.path.exists("data\\cards.pck"):
    cards={}
    for suit in card_suits:
        for value in card_values:
            card_name=value+suit+".png"
            card_dir=os.path.join(imgs_dir,card_name)
            if not os.path.exists(card_dir):
                print(f"!!! {card_dir} does not exist !!!")
                continue
            cards[card_name]=[]
            for f in glob("data\\*.png"):
...
pair_save_dir = "data\\scenes\\val"
trip_save_dir = "data\\scenes\\3val"

if not os.path.isdir(trip_save_dir):
    os.makedirs(trip_save_dir)

if not os.path.isdir(pair_save_dir):
...
for i in tqdm(range(nb_cards_to_generate)):
    ...
    newimg.write_files(pair_save_dir)
...
for i in tqdm(range(nb_cards_to_generate)):
    ...
    newimg.write_files(trip_save_dir)